...
var _JsSpeechRecognizer = __webpack_require__(1);

var _JsSpeechRecognizer2 = _interopRequireDefault(_JsSpeechRecognizer);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var HotwordSpeechRecognition = function () {
  function HotwordSpeechRecognition(_ref) {
    var _this = this;

    var _ref$keywordSpottingM = _ref.keywordSpottingMinConfidence,
        keywordSpottingMinConfidence = _ref$keywordSpottingM === undefined ? 0.5 : _ref$keywordSpottingM,
        _ref$initialTimeout = _ref.initialTimeout,
        initialTimeout = _ref$initialTimeout === undefined ? 5 : _ref$initialTimeout,
        _ref$timeout = _ref.timeout,
        timeout = _ref$timeout === undefined ? 1.5 : _ref$timeout,
        _ref$lang = _ref.lang,
        lang = _ref$lang === undefined ? 'en-US' : _ref$lang,
        _ref$models = _ref.models,
        models = _ref$models === undefined ? {} : _ref$models;

    _classCallCheck(this, HotwordSpeechRecognition);

    this._onKeywordSpottedCallback = function (result) {
      if (!_this._recognizing) {
        _this.onKeywordSpotted(result);
        _this._recognition.start();
      }
    };

    this._onStart = function () {
      _this._recognizing = true;
      _this._stopRecognition(_this.initialTimeoutSecs);
    };

    this._onResult = function () {
      var interimTranscript = '';

      for (var i = event.resultIndex; i < event.results.length; ++i) {
        if (event.results[i].isFinal) {
          _this._finalTranscript += event.results[i][0].transcript;
        } else {
          interimTranscript += event.results[i][0].transcript;
        }
      }
      if (interimTranscript.length) {
        _this.onPartialResult(interimTranscript);
      }
      _this._stopRecognition();
    };

    this._onEnd = function () {
      _this._recognizing = false;
      _this.onFinalResult(_this._finalTranscript);
      _this._finalTranscript = '';
    };

    var hwRecognition = new _JsSpeechRecognizer2.default();
  ...
  _createClass(HotwordSpeechRecognition, [{
    key: 'isSupported',
    value: function isSupported() {
      return 'webkitSpeechRecognition' in window;
    }
  }, {
    key: 'onKeywordSpotted',
    value: function onKeywordSpotted() {
      // overwrite or override
    }
  }, {
    key: 'onPartialResult',
    value: function onPartialResult() {
      // overwrite or override
    }
  }, {
    key: 'onFinalResult',
    value: function onFinalResult() {
      // overwrite or override
    }
  }, {
    key: 'onTrainingStart',
    value: function onTrainingStart() {
      // overwrite or override
    }
  }, {
    key: 'onTrainingStop',
    value: function onTrainingStop() {
      // overwrite or override
    }
  }, {
    key: 'startTraining',
    value: function startTraining(word) {
    ...
    value: function stopTraining() {
      var wordBuffer = this._hwRecognition.wordBuffer;
      var recordingId = this.stop();

      this.onTrainingStop(wordBuffer[wordBuffer.length - 1], recordingId);
      return recordingId;
    }
  }, {
    key: 'playTrainingBuffer',
    value: function playTrainingBuffer(recordingId) {
      return this._hwRecognition.playTrainingBuffer(recordingId);
    }
  }, {
    key: 'deleteTrainingBuffer',
    value: function deleteTrainingBuffer(recordingId) {
      return this._hwRecognition.deleteTrainingBuffer(recordingId);
    }
  }, {
    key: 'setTrainingData',
    value: function setTrainingData(data) {
    ...
    key: '_stopRecognition',


    /* SpeechRecognizer Callbacks End */

    value: function _stopRecognition(timeout) {
      var _this2 = this;

      clearTimeout(this._stopRecognitionTimeout);
      this._stopRecognitionTimeout = setTimeout(function () {
        _this2._recognition.stop();
  ...
  return HotwordSpeechRecognition;
}();

exports.default = HotwordSpeechRecognition;
...
exports.default = JsSpeechRecognizer;
module.exports = exports["default"];

/***/ })
/******/ ]);
});
//# sourceMappingURL=HotwordSpeechRecognition.js.map